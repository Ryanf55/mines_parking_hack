...
[height1, width1, numberOfColorChannels1] = size(RGB);
...
for i = 2:n
    spotCentroid = carBlobs(i).Centroid;
    
    %Q = insertText(Q, spotCentroid, i);
    hold on;
    imshow(prev_img);
   
    pts = [carBlobs(i).BoundingBox(1) carBlobs(i).BoundingBox(2); ...
        carBlobs(i).BoundingBox(1)+carBlobs(i).BoundingBox(3)-1, carBlobs(i).BoundingBox(2); ...
        carBlobs(i).BoundingBox(1)+carBlobs(i).BoundingBox(3)-1, carBlobs(i).BoundingBox(2)+carBlobs(i).BoundingBox(4)-1; ...
        carBlobs(i).BoundingBox(1), carBlobs(i).BoundingBox(2)+carBlobs(i).BoundingBox(4)-1];
    %if (spotsChanged(i) == 1)
    if (vals(i-1) == 1)
        c = 'r';
    else
    ...
    % create mask with bounding box
    % create mask with image Q (og)
    h = drawpolygon('Position', pts, 'Color', c);
